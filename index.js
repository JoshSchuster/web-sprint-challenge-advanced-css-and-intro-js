...
console.log(getArtistByIndex(artists, 0)); //test
  /**


/* Task 4: Create a function called get20s() that takes data as an argument and returns an array with names of artists who were born in and died in 20th century (1900-2000) example born in 1901 and died in 1959 - included / born in 1889 and died in 1925 not included - should return ["Salvador Dali", "Frida Kahlo"]*/

function get20s(arrayData){
  ...
  for(let i = 0; i < arrayData.length; i++){
    ...
    yearBornString = arrayData[i].years.slice(0,4);
    yearDiedString = arrayData[i].years.slice(-4);
    yearBorn = Number(yearBornString);
    yearDied = Number(yearDiedString);

    if((yearBorn >= 1900) && (yearDied <= 2000)){
      returnArray.push(arrayData[i].name); 
    }
  }
  return returnArray;
}

console.log(get20s(artists)); //test
...
function removeArtist(artistsArray, index) {
 ...
 removeArtist(artists, 0); //test 
 console.log(artists[0]); //verify Amedeo is gone
...
function addArtist(objectToAdd){

  artists.push(objectToAdd);
  return artists;
}

const joshArtist = {id: 20,
  name: "Josh Schuster", 
  years: "1989 - Sept 27th, 2020",
  genre: "Web Design", 
  nationality: "Finnish, German, French, English",
  bio: "lorem ipsum"}

  console.log(addArtist(joshArtist)); //test
...
function lotsOfArt(artistsArray){
  ...
  for(let i = 0; i < artistsArray.length; i++){
    if(artistsArray[i].paintings > 100){
      returnArray.push(artistsArray[i].name);
    }
  }
  return returnArray;
}

console.log(lotsOfArt(artists)); //test
...
function randomize(dataArray){
    ...
    for(let i = 1; i < dataArray.length; i++){
      ...
      dataArray[i] = dataArray[randomIndex];
      dataArray[randomIndex] = tempValue;
    }
    return dataArray;
  }

  console.log(randomize(artists)); //test
